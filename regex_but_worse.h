...
typedef enum my_regex_item_class
{
    MY_REGEX_PR,
    MY_REGEX_OPT,
    MY_REGEX_REP,
...
typedef struct my_regex_item
{
    ...
    my_regex_item_class type;                       // item type
...
static void my_regex_item_print(my_regex_item* root, int INDENT);

static inline struct my_regex_item* my_regex_item_init(my_regex_item_class type, const char* name)
{
    ...
    if (name == NULL) {
        switch (type)
        {
            case MY_REGEX_PR: my_regex_error("primitive must have name");
            case MY_REGEX_OPT: 
                k->name = "opt";
                break;
            case MY_REGEX_ALT: 
...
static int my_regex_checker_verify(my_regex_checker* self);
static int my_regex_checker_verify_outer(my_regex_checker* self, my_regex_item* item);
static int my_regex_checker_verify_inner(my_regex_checker* self, my_regex_item* item);

static inline my_regex_checker* my_regex_checker_init(const char* str1)
{
    my_regex_checker* k;
...
static inline int my_regex_item_add_level(my_regex_item* self, my_regex_item* item)
{
...
static inline my_regex_item* my_regex_item_get_child(my_regex_item* self)
{
    switch (self->type)
    {
        case MY_REGEX_ALT:
...
static inline my_regex_item* my_regex_item_index_child(my_regex_item* self, unsigned int index)
{
    if (self->type == MY_REGEX_ALT) {
...
static inline my_regex_item* my_regex_item_add_child(my_regex_item* self, my_regex_item* item)
{
    switch (self->type)
    {
        case MY_REGEX_ALT:
...
static inline my_regex_item* my_regex_item_remove_child(my_regex_item* self)
{
    switch (self->type)
    {
        case MY_REGEX_ALT:
...
static inline int my_regex_item_graft(my_regex_item* self)
{
...
static inline void my_regex_item_print(my_regex_item* root, int INDENT)
{
    my_regex_item_class c = root->type;
...
static inline void my_regex_checker_load(my_regex_checker* self)
{
...
static inline int my_regex_checker_verify_inner(my_regex_checker* self, my_regex_item* expr)
{
    ...
    switch (expr->type)
    {
        case MY_REGEX_ALT:
        {
            ...
            /** switch (expr->levels.count)
            {
                case 1:
                    return my_regex_checker_verify_outer(self, fst);
                case 2: {
                    my_regex_item* snd = my_regex_item_index_child(expr, 1);
                    if (my_regex_checker_verify_outer(self, fst))
                        return 1;
                    else return my_regex_checker_verify_outer(self, snd);
                }
                default: {
                    my_regex_item* snd = my_regex_item_init(MY_REGEX_ALT, NULL);
                    for (int i = 1; i < expr->levels.count; ++i) {
                        my_regex_item* el_i = my_regex_item_index_child(expr, i);
                        if (el_i != NULL) {
                            my_regex_item_add_child(snd, el_i);
                            my_regex_item_add_level(snd, NULL);
                        }
                    }
                    if (my_regex_checker_verify_outer(self, fst)) {
                        return 1;
...
static inline int my_regex_checker_verify_outer(my_regex_checker* self, my_regex_item* expr)
{
    int old = self->index;
...
static inline int my_regex_checker_verify(my_regex_checker* self)
{
